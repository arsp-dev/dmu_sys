...
class BloodNegative:
    ...
    def __init__(self,df: DataFrame):
        ...
        self.org_list = self.blood_gram_negative_organism['ORGANISM'].values.tolist()

    def create_bacterial_pathogens(self):
        x = {}
        for item in self.org_list:
            if len(self.df[self.df['organism'] == item]) > 0:
                x[self.blood_gram_negative_organism['ORG_CLEAN'][self.org_list.index(item)]] = len(self.df[self.df['organism'] == item])
