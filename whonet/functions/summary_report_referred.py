...
pae = pd.read_excel(dirpath + '/whonet/static/whonet_xl/whonet_data_summary_referred.xlsx','Pseudomonas aeruginosa')
aba = pd.read_excel(dirpath + '/whonet/static/whonet_xl/whonet_data_summary_referred.xlsx','Acinetobacter species')
...
def summary_report_referred(file_id,file_name):
    ...
    for value in enterobact_all_list:
        ...
        df_enterobact_all_referred = df_enterobact_all_referred.apply(lambda row: calculate_R_S(row,value,df_enterobact_all_referred,enterobact_all_list), axis = 1)
    ...
    if len(df_enterobact_all) > 0:
        ...
        # df_enterobact_all.dropna(subset = df_enterobact_all.columns, inplace=True)
        df_enterobact_all.dropna(how = 'all',inplace = True)
        df_enterobact_all = df_enterobact_all.drop(columns=['ORIGIN_REF','FILE_REF','ID','comp','ent_fast'])
