...
class EfaEfm:
    ...
    def process(self) -> pd.DataFrame:
        ...
        if len(df) > 0:
            ...
            df, cols = remove_null_cols(df,['Test','INSTITUT','LABORATORY','STOCK_NUM','FIRST_NAME','LAST_NAME','PATIENT_ID','SEX','AGE','DATE_BIRTH','DATE_ADMIS','SPEC_NUM','SPEC_DATE','SPEC_TYPE','ORGANISM','X_REFERRED','ESBL','AMK_ND30','GEH_ND120','GEH_NM','GEH_RIS','STH_ND300','STH_NM','STH_RIS','LNZ_ND30','LNZ_NM','LNZ_RIS','VAN_ND30','VAN_NM','VAN_RIS','DAP_NM','DAP_RIS','TZD_ND','TZD_NM','TZD_RIS'])
            df = df[cols]
            return df
        return df
