...
from whonet.functions.summary_report_helper import get_date_to_compute, calculate_R_S, calculate_R_S_MIC, remove_null_cols, clean_sau, clean_sau_days
dirpath = os.getcwd()
...
class Sau:
    ...
    def __init__(self, df : pd.DataFrame, num_of_days : int = 3) -> None:
        self.df = df[df['ORGANISM'].isin(['sau'])]
    ...
    def process(self) -> pd.DataFrame:
        ...
        if len(df) > 0:
            ...
            df, cols = remove_null_cols(df,['Test','PATIENT_ID','SEX','AGE','DATE_BIRTH','DATE_ADMIS','SPEC_NUM','SPEC_DATE','SPEC_TYPE','ORGANISM','X_REFERRED','ESBL','INDUC_CLI','FOX_ND30','FOX_NM','FOX_RIS','OXA_NM','OXA10.10_RIS','LNZ_ND30','LNZ_NM','LNZ_RIS','DAP_NM','DAP_RIS','TZD_ND','TZD_NM','TZD_RIS'])
            df = df[cols]
            return df
        return df
    ...
    def resistant_clean_sau_days(self, df: pd.DataFrame) -> pd.DataFrame:
        return df.apply(lambda row: clean_sau_days(row), axis = 1)
    
    def resistant_clean_sau(self, df: pd.DataFrame) -> pd.DataFrame:
        return df.apply(lambda row: clean_sau(row),axis = 1)
