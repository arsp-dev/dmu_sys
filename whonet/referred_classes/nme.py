...
class Nme:
    ...
    def process(self) -> pd.DataFrame:
        ...
        if len(df) > 0:
            ...
            df, cols = remove_null_cols(df,['Test','INSTITUT','LABORATORY','STOCK_NUM','PATIENT_ID','FIRST_NAME','LAST_NAME','SEX','AGE','DATE_BIRTH','DATE_ADMIS','SPEC_NUM','SPEC_DATE','SPEC_TYPE','ORGANISM','X_REFERRED','ESBL','CIP_ND5','CIP_NM','CIP_RIS','MEM_ND10','MEM_NM','MEM_RIS','RIF_ND5','RIF_NM','RIF_RIS','CRO_ND30','CRO_NM','CRO_RIS','AMP_ND10','AMP_NM','AMP_RIS','PEN_NM','PEN_RIS'])
            df = df[cols]
            return df
        return df
